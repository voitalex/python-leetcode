...
The algorithm for myAtoi(string s) is as follows:
  ...
  * If the integer is out of the 32-bit signed integer range [-231, 231 - 1], then clamp the integer so that
    it remains in the range. Specifically, integers less than -2^31 should be clamped to -2^31,
...
class Solution:
    ...
    def _sign(self, string: Optional[str]) -> int:
    ...
    def myAtoi(self, s: str) -> int:
        ...
        if not (search := re.search(r'\s*([\-\+]?)([\.\+\-\w]+)', s)):
            return 0

        result = 0
        sign = self._sign(search.group(1))

        for char in (itertools.takewhile(lambda ch: ch.isdigit(), search.group(2))):

            digit = int(char)

            # Если мы в шаге от переполнения числа, то возвращает минимальное или максимальное число
            if result > self.MAX_DIV_10 or (result == self.MAX_DIV_10 and digit >= 8):
                return self.MAX_INT if sign > 0 else self.MIN_INT
